...
void mont_inv(mpz_t &r, const mpz_t &a, ECParameters &param) {
    mpz_invert(r, a, param.p);
    mont_mul(r, r, param._r3, param);
}

void mont_pow(mpz_t &r, const mpz_t &a, const mpz_t &n, ECParameters &param) {
    THREAD_LOCAL_MPZ_WORKSPACE(n1, t1, base);
    
    mpz_set(n1, n);
    mpz_set_ui(r, 1);
    if (mpz_sgn(n1) < 0) {
        mpz_neg(n1, n1);
        mont_inv(base, a, param);
    } else {
        mpz_set(base, a);
    }
    ...
    for (int i = 0; i < n_bits; i++) {
        if (mpz_tstbit(n1, i)) {
            mont_mul(r, r, t1, param);
        }
        mont_sqr(t1, t1, param);
    }
}
